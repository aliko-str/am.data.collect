...
function doEachCategoryRecursively(allUrlObj, _catCounter, allResObj, cbAllCatsDone){
	...
	amListings.processUrlBatch(allUrlObj[catNames[_catCounter]], function(oneCatResults){
		...
		if((_catCounter + 1) < catNames.length){
			doEachCategoryRecursively(allUrlObj, _catCounter + 1, allResObj, cbAllCatsDone);
		}else{
			cbAllCatsDone(allResObj);
		}
	});
}
...
function runItAll(state) {
	...
	doEachCategoryRecursively(amUrl.getAllUrlAsObj(), 0, {}, function(allResObj){
		aReqAndSave.saveBookArrInTabTables(allResObj, amUrl.AmazonBookPiece1.getHeader());
